...
public:
  SRCOdom() : Node("ackermann_odometry")
  {
    ...
    RCLCPP_INFO(get_logger(), "is_gazebo : %s", is_gazebo_ == true ? "true" : "false");
    ...
    joint_state_sub_ = this->create_subscription<JointState>("joint_states", 10,
                                                             std::bind(&SRCOdom::joint_state_cb, this, std::placeholders::_1));
  ...
  void joint_state_cb(const JointState::SharedPtr msg)
  {
    ...
    for (int i = 0; i < 6; i++)
    {
      if (msg->position[i] == 0.0 || fabs(msg->position[i]) > 100000000)
        break;
    ...
    if (verbose_)
      RCLCPP_INFO(this->get_logger(), "left_rear_wheel_joint : %f / right_rear_wheel_joint : %f / Rear Wheel Pose : %f", 
        left_rear_wheel_joint, right_rear_wheel_joint, rear_wheel_pos);
  }

  template <typename T>
  bool update_odom(T encoder_pos, double heading, const rclcpp::Time &cur_time)
  {
    T rear_encoder_diff = (encoder_pos - prev_encoder_pos);
    T rear_wheel_diff;
    if (is_gazebo_ == true)
      rear_wheel_diff = rear_encoder_diff * wheel_radius_;
    else
      rear_wheel_diff = rear_encoder_diff * (2 * M_PI * wheel_radius_ / encoder_resolution_);
    ...
    // encoder값이 너무 튀어서 사용할 수 없는 정도임
    // 그래서 gazebo에서는 rolloing windows를 썼나보다.
  ...
  void timer_cb()
  {
    ...
    if (is_gazebo_ == true)
    {
      if (std::isnan(rear_wheel_pos))
        return;

      // TODO : rolling window 
      update_odom(rear_wheel_pos, this->heading_, time);
    }
    else
